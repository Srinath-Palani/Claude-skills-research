...
This repository includes automated quality checks to ensure documentation always stays in sync with code changes:

**3-Part Automation System:**

1. **Pre-commit Hook** — Blocks commits if SKILL.md changes without CLAUDE.md or README.md
   - Prevents documentation drift automatically
   - Shows clear error message with fix instructions
   - Located: `.git/hooks/pre-commit`

2. **Commit-Msg Hook** — Enforces structured commit messages
   - Validates `Type: Description` format (Feature/Fix/Update/Docs/etc)
   - Recommends Co-Authored-By line
   - Rejects vague or empty messages
   - Located: `.git/hooks/commit-msg`

3. **Commit Template** — Guides structured, consistent messages
   - Auto-loads when you run `git commit`
   - Shows required fields and examples
   - Located: `.git-commit-template`
...
1. **CLAUDE.md** — Update if:
   ...
   - Registry or skill system improvements
...
3. **Proper Commit Workflow:**
   ```bash
   # 1. Make your changes
   vi skills/unified-mcp-skill/SKILL.md
   ...
   # 4. Commit (template auto-loads)
   git commit  # Opens editor with template guidance

   # 5. Push to GitHub
   git push origin main
   ```

4. **Template-Guided Format:**
   ```
   Feature: Add new MCP research capability

   Updates:
   - CLAUDE.md: Added new feature section
   - README.md: Updated getting started
   - SKILL.md: Integrated new learning

   Key Changes:
   - Enhanced endpoint verification
   - Added security scoring
